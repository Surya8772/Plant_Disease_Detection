...
ADMIN_PASS_HASH = hashlib.sha256("admin123".encode()).hexdigest() # Change password here
...
if st.session_state.role == "admin":
    if menu == "📊 Admin Dashboard":
        st.title("🛡️ Admin Dashboard - KrishiRakshak")
        ...
        if os.path.exists("appointments.csv"):
            ...
            st.subheader("📈 Disease-wise Bookings")
        ...
        if os.path.exists("users.csv"):
            ...
            st.download_button("Download Users CSV", df.to_csv(index=False), "users.csv")
        else: st.info("No users yet")

    elif menu == "📋 All Appointments":
        st.title("📋 All Appointments (All Farmers)")
        if os.path.exists("appointments.csv"):
            df = pd.read_csv("appointments.csv")
            # Filters
            f1, f2 = st.columns(2)
            doctor_filter = f1.selectbox("Filter by Doctor", ["All"] + [d["name"] for d in DOCTORS])
            status_filter = f2.selectbox("Filter by Status", ["All", "Confirmed"])
            if doctor_filter!= "All": df = df[df['Doctor']==doctor_filter]
            st.dataframe(df, use_container_width=True)
            st.download_button("Download Appointments CSV", df.to_csv(index=False), "appointments.csv", type="primary")
            if st.button("🗑️ Clear All Appointments"):
...
# --- USER PANEL (Same as before) ---
    ...
    if menu == "📊 Dashboard":
        ...
        total_appts = len(pd.read_csv("appointments.csv")) if os.path.exists("appointments.csv") else 0
        c1.metric("My Appointments", total_appts)
        ...
        if st.session_state.last_booking:
            ...
            if sent: c2.success(f"✅ Email sent to {doctor_obj['email']}")
